...
// setupCRISandboxImage rewrite the containerd config file to use the sandbox image recommended by kubeadm
func setupCRISandboxImage(bc *bits.BuildContext) error {
	...
	if len(images[0]) > 0 {
		...
		if currentSandboxImage != images[0] {
			log.Infof("updating the config file %s to use the recommended sandbox image %s", tmpConfigFileOnHost, images[0])
